...
public class CrateSettings {
    ...
    private final boolean fatePointEnabled, overrideEnabled;
    ...
    public CustomItemStack find(boolean isLimited, boolean all, String itemName) {
        Rarity rarity = Rarity.LEGENDARY;
        if (all) {
            Set<CustomItemStack> temp = new HashSet<>();
            temp.addAll(standard.get(rarity));
            temp.addAll(limited.get(rarity));
            return temp.stream().filter(item -> item.getString("itemName").equals(itemName)).findFirst().orElse(null);
        }

        if (isLimited) {
            return limited.get(rarity).stream().filter(item -> item.getString("itemName").equals(itemName)).findFirst().orElse(null);
        } else {
            return standard.get(rarity).stream().filter(item -> item.getString("itemName").equals(itemName)).findFirst().orElse(null);
        }
    }
}
